...
1. **Perplexity Search** - For current best practices and alternatives:
   ```bash
   cd ~/.claude/skills/perplexity-search
   uv run --with aiohttp python scripts/perplexity_search.py --research "best practices for [topic] 2026"
   ```

2. **NIA Documentation** - For library-specific guidance:
   ```bash
   cd ~/.claude/skills/nia-docs
   uv run --with aiohttp python scripts/nia_docs.py search package [library] --registry py_pi --query "[topic]"
   ```
